...
bool conditia(TElem e) {
    return e % 2 == 0; // conditie pentru a pastra doar numerele pare
}
...
void testFiltrare() {
    ...
    m.filtreaza(conditia); //filtram numerele impare
    assert(m.dim() == 50); //ar trebui sa ramana doar numerele pare
    IteratorMultime im = m.iterator();
    while (im.valid()) {
        assert(im.element() % 2 == 0); //verificam ca toate elementele sunt pare
        im.urmator();
    }
    assert(im.valid() == false);
}
