...
void testCos() {
    ...
    const std::string numeFisier1 = "../cos.html";
    cos.exportHtml(numeFisier1);
...
void testGolesteCos() {
    ...
    const auto& cartiRepo = repo.getCarti();
    assert(cartiRepo.getSize() == 2);

    assert(cartiRepo[0].getTitlu() == "Ion");
    assert(cartiRepo[1].getTitlu() == "Morometii");
...
void testAdaugaCarteCos() {
    ...
    try {
        service.adaugaCarteDupaTitlu("Carte Inexistenta");
    } catch (const RepoException &e) {
        std::cout << "Exceptie prinsa: " << e.what() << std::endl;
    }
...
void testGenereazaCosSiExport() {
    ...
    Lista<Carte> carti = repo.getCarti();
    ...
    const std::string numeFisier1 = "../cos.html";
    service.exportHtml(numeFisier1);
    std::ifstream fisier1(numeFisier1);
    assert(fisier1.is_open());

    std::string linie1;
    int count1 = 0;
    while (std::getline(fisier1, linie1)) {
        count1++;
    }
    std::cout << "Numarul de linii din fisier HTML: " << count1 << std::endl;
    assert(count1 == 15);
    fisier1.close();
