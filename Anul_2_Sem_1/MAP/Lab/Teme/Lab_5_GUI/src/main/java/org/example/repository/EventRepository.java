...
public class EventRepository implements Repository<Long, RaceEvent> {
    ...
    public Optional<RaceEvent> findById(Long id) {
        ...
        try (Connection conn = getConnection();
            ...
            List<SwimmingDuck> ducks = loadEventParticipants(eventId, conn);
            List<Lane> lanes = loadEventLanes(eventId, conn);

            RaceEvent event = new RaceEvent(eventId, ducks, lanes);
            List<User> subscribers = loadEventSubscribers(eventId, conn);
            ...
            System.out.println("Error finding event by ID: " + e.getMessage());
        }
    ...
    public Iterable<RaceEvent> getAll() {
        ...
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                ...
                List<SwimmingDuck> ducks = loadEventParticipants(eventId, conn);
                List<Lane> lanes = loadEventLanes(eventId, conn);
                list.add(new RaceEvent(eventId, ducks, lanes));
            }
    ...
    private List<Lane> loadEventLanes(Long eventId, Connection conn) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                lanes.add(new Lane(laneId, distance));
            }
        }
    ...
    """;
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                userRepo.findById(uid).ifPresent(users::add);
            }
        }
